...
, Game.TileSheet:getWidth(), Game.TileSheet:getHeight())
  ...
  Game.TileTypes[21] = "Mer"

  print("GAME:Chargement textures terminée...")
  
  print("GAME:Création brouillard ...")
  Game.Map.FogGrid = {}
  local l,c
  ...
  print("GAME:Création brouillard terminée...")
...
function Game.Draw()
 ...
 do 
      ...
      if textureQuad ~= nil then
        ...
        love.graphics.draw(Game.TileSheet,textureQuad,x,y)
...
function Game.Map.IsSolid(pId)
  ...
  if typeTile == "Rocher" or typeTile == "Arbre" or typeTile == "Mer" or typeTile == "Lave" then
    return true
  end
  return false
end
...
function Game.Map.ClearFog(pLine, pColumn)
  local l,c
  for  l = (pLine - 1), (pLine + 1) do
    for c = (pColumn - 1), (pColumn + 1) do 
...
function Game.Map.ClearFog2(pLine, pColumn)
  local l,c
  for l = 1, Game.Map.MAP_HEIGHT do 
    for c = 1, Game.Map.MAP_WIDTH do 
       if c > 0 and c <=  Game.Map.MAP_WIDTH and l > 0 and l <= Game.Map.MAP_HEIGHT then
        local dist = math.dist(l,c,pLine,pColumn)
        if dist < 5 then
          local alpha = dist / 5
